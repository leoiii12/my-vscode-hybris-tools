...
export namespace Config {
  export function getHacUrl(): string {
    if (vscode) {
      return (
        vscode.workspace
          .getConfiguration()
          .get('vscode-hybris-tools.hac.url') || 'https://localhost:9012/hac/'
      )
    }
  ...
  export function getHacUsername(): string {
    if (vscode) {
      return (
        vscode.workspace
          .getConfiguration()
          .get('vscode-hybris-tools.hac.username') || 'admin'
      )
    }
  ...
  export function getHttpTimeout(): number {
    if (vscode) {
      return (
        vscode.workspace
          .getConfiguration()
          .get('vscode-hybris-tools.http.timeout') || 10 * 1000
      )
    }
  ...
  export function getHttpUseStrictSSL(): boolean {
    if (vscode) {
      return (
        vscode.workspace
          .getConfiguration()
          .get('vscode-hybris-tools.http.useStrictSSL') || false
      )
    }
  ...
  export function getOfflineTypeCodes(): string[] {
    if (vscode) {
      return (
        vscode.workspace
          .getConfiguration()
          .get('vscode-hybris-tools.offline.typeCodes') || []
      )
    }
